...
_start:
    mov eax, msg_func      ; Выводим сообщение с описанием функции
    call sprintLF

    pop ecx                ; Получаем количество аргументов
    pop edx                ; Пропускаем имя программы
    sub ecx, 1             ; Уменьшаем на 1, так как имя программы не считается
    mov esi, 0             ; Переменная для суммы результата

next:
    cmp ecx, 0h            ; Если все аргументы обработаны, переходим к завершению
    jz _end
